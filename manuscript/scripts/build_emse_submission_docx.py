...
SOURCE = ROOT / "manuscript" / "Limits_of_Task_Set_Reduction_EMSE_submission.md"
OUTPUT = ROOT / "manuscript" / "Limits_of_Task_Set_Reduction_EMSE_submission.docx"
...
def configure_styles(doc):
    ...
    for style_name, size, color, before, after in (
        ("Heading 1", 14, INK, 14, 8),
        ("Heading 2", 12, INK, 10, 5),
...
def add_inline(paragraph, text):
    ...
    for match in token_re.finditer(text):
        ...
        if token == "τ_b":
            ...
            run = paragraph.add_run(token[1:-1])
            set_font(run, name="Courier New", size=9, color=INK)
        else:
...
def add_title(doc):
    ...
    r = p.add_run(
        ...
        "A Temporal Study of Leaderboard Ranking Reliability"
    )
...
def add_table(doc, rows):
    ...
    for i, data_row in enumerate(rows):
        for j, value in enumerate(data_row):
            ...
            add_inline(p, value)
            for run in p.runs:
                set_font(run, size=8, color=INK, bold=(i == 0))
...
def build():
    ...
    i = 1  # skip the source title; the submission title is inserted above
    in_references = False
    while i < len(lines):
        ...
        # Keep Table 4 intact. Its position can otherwise leave the caption at
        # the foot of a page, separated from the decision rows.
        if text.startswith("**Table 4."):
